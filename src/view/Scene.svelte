<script>
import { fade, fly } from 'svelte/transition';
import { audioStore, youtubeStore, playTrigger, playAudio } from '../logic/store';

import Sky from './svg/Sky.svelte';
import Sun from './svg/Sun.svelte';
import Text from './svg/Text.svelte';
import TextInverted from './svg/TextInverted.svelte';
import Water from './svg/Water.svelte';
import Reflection from './svg/Reflection.svelte';

$:playing = $audioStore.status === 'playing';
$:playable = $youtubeStore.playable;

const handleClick = ()=> {
  $playAudio = true;
  $playTrigger *= -1;
}
</script>

<!-- Svelte has restrictions on what can be applied to non-DOM elements
     SVG has restrictions on what styles can be applied to its elements
     ∴ containers in containers and lots of <g>
-->
<div class="scene-container-container" class:playable on:click={handleClick}>
<div class="scene-container" class:playing>
  <svg viewBox="0 0 100 100">
    <defs>
      <clipPath id="clip">
        <circle cx="50" cy="50" r="50"/>
      </clipPath>
    </defs>

    <g style="clip-path: url(#clip);">
      <Sky />

      {#if $youtubeStore.playable}
        <g in:fly={{ y: 10, duration: 1500 }} out:fade={{ y: 200, duration: 500 }}>
          <Sun />
        </g>
      {/if}

      <Text />
      <Water />   
      <TextInverted />

      {#if $youtubeStore.playable && $audioStore.timeRanges.length > 0}
        <g transition:fade={{ duration: 500 }}>
          <Reflection />
        </g>
      {/if}
    </g>
  </svg>
</div>
</div>

<style>

.scene-container-container {
  clip-path: circle(50% at center);
  overflow: hidden;
}

.scene-container::after {
  content: '';
  position: fixed;
  width: 100%;
  height: 100%;
  left: 0%;
  top: 0%;
  background-image: url(data:image/png;base64,);
  overflow: hidden;
}

.scene-container.playing::after {
  animation: grain 1.5s steps(3) infinite;
}

@keyframes grain {
  from, to { background-position: initial }
  10% { background-position: left -5% top 5%; }
  30% { background-position: left 7% top -7%; }
  50% { background-position: left -5% top 5%; }
  70% { background-position: left 3% top -3%; }
  90% { background-position: left 5% top -5%; }
}

.scene-container-container.playable:hover {
  cursor: pointer;
}

</style>